...
public class GameView {
    ...
    private void renderLevel() {
        ...
        ArrayList<LevelNode> level = generateLevel(200);                         // TODO: Fetched from match server
...
//        double marginX = 0.2, marginY = 0.125; // Với mỗi ô block, có margin ngang và dọc (điều chỉnh tương ứng với tỉ lệ màn hình)
        double marginX = 0.5, marginY = 0.5; // Với mỗi ô block, có margin ngang và dọc (điều chỉnh tương ứng với tỉ lệ màn hình)
        ...
        int countPerfect = (int) Math.pow(blockPerRow, 2);
            ...
            for (int j = 0; j < blockPerRow; j++) {                  // Lặp tuần tự từ trên xuống dưới, từ trái qua phải
                ...
                levelNode.setCoord(new Point2D.Double(
                        (
                            ...
                            ) * ((double) blockPerRow / countPerfect), // Cuối cùng, đổi thành tỉ lệ phù hợp với count sao cho LevelNode nằm trong khoảng [0,1]
                        (
                                j
                                + (
                                    marginY
                            ...
                            ) * ((double) blockPerRow / countPerfect)
                ));

                System.out.println(String.format("i = %s, j = %s; (%s; %s)", i, j, levelNode.getCoord().x, levelNode.getCoord().y));
